...
template <typename FromType, typename ToType,
          ...
          typename std::enable_if_t<std::is_same_v<std::decay_t<ToType>, detail::Array> &&
...
template <typename FromType, typename ToType,
          ...
          typename std::enable_if_t<std::is_reference_v<ToType> &&
                                    std::is_same_v<std::remove_cv_t<std::remove_reference_t<ToType>>,
                                                   detail::Array>> * = nullptr>
static ToType generic_cast(detail::VM vm, FromType &&from) {
    if (from._type != tagSQObjectType::OT_ARRAY) {
        throw sqbind17::value_error("unsupported value");
    }
    thread_local std::unique_ptr<detail::Array> value;
    value = std::make_unique<detail::Array>(_array(from), vm);
    return *value;
}
